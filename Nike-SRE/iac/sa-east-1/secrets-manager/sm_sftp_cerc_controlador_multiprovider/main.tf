terraform {
  backend "s3" {
    key = "sa-east-1/secrets-manager/sm_sftp_cerc_controlador_multiprovider/terraform.tfstate"
  }
}
...
module "sm_sftp_cerc_controlador_multiprovider" {
  ...
  name                    = "sm_sftp_cerc_controlador_multiprovider"
  secret_string           = "pI60q@od38"
