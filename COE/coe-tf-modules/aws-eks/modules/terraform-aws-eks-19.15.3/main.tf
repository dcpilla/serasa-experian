...
module "kms" {
  # Fix the issue with 403 return when run git clone
...
resource "aws_eks_addon" "before_compute" {
  ...
  configuration_values        = try(each.value.configuration_values, null)
  preserve                    = try(each.value.preserve, null)
  resolve_conflicts_on_create = try(each.value.resolve_conflicts_on_update, "OVERWRITE")
  service_account_role_arn    = try(each.value.service_account_role_arn, null)
  ...
  node_iam_role_arns_non_windows = distinct(
    compact(
      concat(
        ...
        [for group in module.eks_managed_node_group_infra : group.iam_role_arn],
