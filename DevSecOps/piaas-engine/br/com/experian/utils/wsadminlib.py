...
def getTargetsForApplication(appname, targettype):
    ...
    if (len(deployments) > 0) :
        ...
        #sop(m, "\n\nrawDeploymentTargets=%s\n\n" % ( repr(rawDeploymentTargets) ))
        # Convert the single string to a real python list containing strings.
        deploymentTargetList = stringListToList(rawDeploymentTargets)
        #sop(m, "\n\ndeploymentTargetList=%s\n\n" % ( repr(deploymentTargetList) ))

        # Handle each target mapping...
        for targetMapping in targetMappingList:
            #sop(m, "\n\ntargetMapping=%s" % targetMapping)
            targetMappingTarget = getObjectAttribute(targetMapping,"target")
            #sop(m,"targetMapping=%s targetMappingTarget=%s" % ( targetMapping, targetMappingTarget ))
            #print("\n\ntargetMappingTarget.find(targettype) = %s\n\n" % targetMappingTarget.find(targettype))
...
def updateApplication(filename,appname):
    ...
    #options = ['-MapWebModToVH', [['.*', '.*', 'default_host']]]
